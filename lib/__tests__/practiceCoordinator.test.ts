...
import type { PracticeCommand } from "@/shared/practiceMachine";
...
function lifecycleCommand(): PracticeCommand {
  return {
    kind: "recordBreakerLifecycle",
    id: "breaker:attempt-1:repairCompleted",
    domain: "breaker-lifecycle:attempt-1",
...
describe("review regression: remount must not defeat exclusivity", () => {
  it("replays a pending lifecycle command after an in-process remount releases its claim", async () => {
    const command = lifecycleCommand();
    const firstMount = acquirePracticeCoordinator(SCHOLAR);
    let release!: () => void;
    const gate = new Promise<void>((resolve) => {
      release = resolve;
    });
    const runs: string[] = [];
    const running = firstMount.execute(command, async () => {
      runs.push("first");
      await gate;
    });
    releasePracticeCoordinator(SCHOLAR);
    ...
    expect(remounted).toBe(firstMount);
    expect(await remounted.execute(command, async () => {
      runs.push("overlap");
    })).toBe(false);
    ...
    expect(runs).toEqual(["first", "replay"]);
  });
