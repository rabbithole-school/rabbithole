...
export const aiGetProposalContext = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    // The scholar's OWN institution identity — the proposal generator identifies
    // with the child's actual school, not a hardcoded primary name (Rabbithole is
    // multi-tenant, and this output can reach the scholar's tutor context).
    const institutionProfile = await institutionPromptProfileForScholar(
      ctx,
      doc.scholarId,
    );
    ...
    return {
      document: {
        ...
        // Proposals become directives + seeds that land in the SCHOLAR-facing
        // tutor prompt, so feed the redacted (number-free) variant here — never
        // the score-bearing summary/keyFindings (those are teacher-only).
        redactedSummary: doc.redactedSummary ?? null,
        keyFindings: doc.redactedKeyFindings ?? [],
      },
      institutionProfile,
...
export const generateProposal = authedMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await requireActiveScholarAccess(ctx, ctx.user, doc.scholarId);
...
export const rejectProposal = teacherMutation({
  ...
  handler: async (ctx, args) => {
    const proposalRow = await ctx.db
      ...
      .first();
    if (!proposalRow) throw new Error("No proposal cached for this document");
    await requireActiveScholarAccess(ctx, ctx.user, proposalRow.scholarId);
