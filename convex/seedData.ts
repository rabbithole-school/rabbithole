...
import { insertProdUnits } from "./seedProdUnits";
import { insertProbabilityStrand } from "./seedProbabilityStrand";
import { insertAiLiteracyUnits } from "./seedAiLiteracyUnits";
import { insertGeographyQuests } from "./seed/geographyQuests";
import { insertGamesDemoUnit } from "./seed/gamesDemo";
import { insertSystemsAgentsUnit } from "./seed/systemsAgents";
import { insertCooperationConflictUnit } from "./seed/cooperationConflict";
...
/** Slug for a devLogin username: lowercase, `[a-z0-9_]` only, no edge `_`. */
function devUsernameSlug(raw: string): string {
  return raw
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "_")
    .replace(/^_+|_+$/g, "");
...
/**
 * Give EVERY username-less user a stable `username` so they're `/dev-login`-able
 * (the picker lists only users with a username). Idempotent + field-level, so it
 * runs safely on every seed — including when the bulk seed is skipped
 * (already-seeded deployment). This is the fix for the "skip-if-exists seed never
 * backfills a new field" trap, now generalized: not just the seed's own test
 * users (which carry an `externalId`) but anyone created by other seed paths —
 * operations staff, parents, and scholars from private enrollment fixtures,
 * who have no `externalId` — also become loginable on dev.
 *
 * The handle prefers `externalId` (test ids / OAuth subject), then the email
 * local-part, then the display name, then the role; collisions get a numeric
 * suffix. `isSystem` accounts are skipped (kept non-loginable, e.g. the
 * onboarding owner "rabbithole-guide").
 *
 * NEVER runs on prod. `username` is the password-login resolution key — the
 * Password provider adopts an existing account by matching `username` (see
 * `convex/auth.ts` createOrUpdateUser), so a username on a real, passkey-less
 * account is an account-claim coupon. Gating to non-prod removes the landmine
 * where a `db:seed:prod` would spray claimable usernames onto live accounts.
 * (Prod onboarding — `seed/enrolledFamilies`, `ensureStaffAccount` — therefore
 * deliberately sets no username; see TODO.html "Rationalize how scholars/parents
 * claim accounts on prod".)
 */
async function reconcileDevUsernames(ctx: MutationCtx): Promise<void> {
  ...
  for (const u of users) if (u.username) taken.add(u.username);

  let patched = 0;
  for (const u of users) {
    if (u.username || u.isSystem) continue;
    // Prefer an explicit externalId VERBATIM — it's already a valid handle
    // (e.g. "test-teacher-001" / an OAuth subject), and the canonical seeded
    // usernames keep their hyphens. Otherwise derive a slug from the email
    // local-part, then the display name, then the role.
    const base =
      (u.externalId && u.externalId.trim()) ||
      devUsernameSlug(u.email ? u.email.split("@")[0] : "") ||
      devUsernameSlug(u.name ?? "") ||
      (u.role ?? "user");
    let candidate = base;
    let n = 1;
    while (taken.has(candidate)) candidate = `${base}_${++n}`;
    taken.add(candidate);
    await ctx.db.patch(u._id, { username: candidate });
    patched++;
  }
  if (patched > 0) console.log(`Assigned ${patched} devLogin username(s).`);
}

export const clearAll = internalMutation({
  handler: async (ctx) => {
    const tables = [
      "sessions", "messages", "analyses", "observations",
      "masteryObservations", "teacherMasteryOverrides", "seeds",
      "sessionSignals", "crossDomainConnections", "personas", "perspectives",
      "units", "assignments", "processes", "artifacts", "processState",
      "users",
    ] as const;
...
export const seedAll = internalMutation({
  handler: async (ctx) => {
    // ALWAYS reconcile devLogin-able usernames first — even when the bulk
    // seed is skipped below — so a deployment seeded before usernames existed
    // still gets them (the docs/SOP promise `test-teacher-001` etc.).
    await reconcileDevUsernames(ctx);

    // Authored Launchpad content is ALWAYS reconciled, even when the bulk seed
    // is skipped below: `db:reset` wipes `instructionContent`, nothing else
    // refills it, and an empty table makes the practice Launchpad invisible for
    // every scholar in dev (the selector's `hasContent` gate). Idempotent.
    await seedAuthoredInstructionInto(ctx);

    // Launch-arc World content is reconciled before the bulk-seed sentinel so
    // existing private dev deployments receive newly added lessons/activities.
    const existingUsers = await ctx.db.query("users").collect();
    const existingWorldTeacher =
      existingUsers.find((user) => user.username === "test-teacher-001") ??
      existingUsers.find((user) =>
        user.role === ROLES.TEACHER || user.role === ROLES.PLATFORM_ADMIN
      );
    if (existingWorldTeacher) {
      const result = await insertSystemsAgentsUnit(ctx, existingWorldTeacher._id);
      console.log(
        `Systems & Agents reconciled: ${result.lessonsCreated} lesson(s), ${result.activitiesCreated} activity(s).`,
      );
      const coopResult = await insertCooperationConflictUnit(ctx, existingWorldTeacher._id);
      console.log(
        `Cooperation & conflict reconciled: ${coopResult.lessonsCreated} lesson(s), ${coopResult.activitiesCreated} activity(s).`,
      );
    }
    ...
    if (existingPersonas) {
      console.log("Already seeded, skipping bulk seed (usernames reconciled).");
      return;
    }

    // Create platform-admin account (used as FK for seeded entities). A
    // FICTIONAL stand-in for the real founder — dev data must not carry real
    // identities (see seed/devPersonas.ts, which also makes this platform admin a parent
    // of Oliver Stone).
    ...
    // ── Seed Personas ──────────────────────────────────────────────
    // DEPRECATED (anti-parasocial, 2026-06): persona rows are still seeded so
    // existing data/history survive and re-enabling is trivial, but NO seeded
    // unit wires a persona anymore (the unit insert no longer copies personaId)
    // and the tutor never injects one. Personas are also hidden from active
    // teacher/scholar surfaces. See TODO.html ("Reimagine personas").
...
You are energetic, optimistic, and hands-on. You love the messiness of the creative process and help scholars see that designing is thinking — not just making things look nice. You say things like "Let's try it and see" and "What if we flipped that around?"`,
    ...
    const personaIdByTitle: Record<string, Id<"personas">> = {};
...
Guide them to apply this lens to everything — not just politics. A school rule, a game's design, a pricing model, a historical event, a scientific funding decision, a social media algorithm — everything exists because someone made choices, and those choices serve some interests more than others. This lens builds critical thinking without cynicism: understanding incentives doesn't mean everything is corrupt, but it does mean nothing is accidental.`,
    ...
    const perspectiveIdByTitle: Record<string, Id<"perspectives">> = {};
...
The document is plain text only. The document title becomes the story title.`,
    ...
    // Seed only a few representative shell units — most of the dev seed's
    // realistic curriculum now comes from the 3 complete prod units below
    // (insertProdUnits). The rest of unitDefs stays as a menu to draw from
    // (e.g. seedUnitsAndTopics) without crowding a fresh deployment.
    const KEEP_SHELL_UNITS = new Set([
      "Animal Adaptations",
      "Prime Numbers",
      "Kitchen Chemistry",
      "Bug Hunt",
    ]);
    for (const u of unitDefs.filter((u) => KEEP_SHELL_UNITS.has(u.title))) {
      await ctx.db.insert("units", {
        ...
        // DEPRECATED (anti-parasocial): seeded units no longer wire a persona —
        // the tutor must not act as a character. Persona rows are still seeded
        // (below) so existing data + history survive and this is reversible, but
        // nothing references them. See TODO.html ("Reimagine personas").
    ...
    const testUsers = [
      {
        ...
        email: "test.teacher@moli.school",
        name: "Test Teacher",
    ...
    for (const u of testUsers) {
      await ctx.db.insert("users", {
        ...u,
        // Give each a stable username (= externalId) so they're devLogin-able
        // and resolvable by a unique key in tests / the /dev-login handoff
        // (e.g. test-teacher-001, test-scholar-001). Display names collide
        // (multiple "Kai"s across seeds); usernames don't.
    ...
    const processIdByTitle: Record<string, Id<"processes">> = {};
...
- G (Grow): What did you learn? How will you prevent this next time? What does this teach you about how the system works? "The best debuggers don't just fix the bug — they understand why it happened."`,
    ...
    // The 3 complete prod units (Aquaponics QUEST, SpaceX & the IPO Question,
    // Autorotation) — full lesson + activity trees, the realistic curriculum a
    // fresh deployment should have.
    const prodUnitCount = await insertProdUnits(ctx, systemTeacherId);

    // The hand-authored Probability strand (Chance & Likelihood, Counting the
    // Odds, Rolling the Truth) — gives the probability practice domain a home in
    // the curriculum, with a `problem_set` activity per unit that serves the
    // tactile dice/coin manipulative. Idempotent by slug.
    const probabilityUnitCount = await insertProbabilityStrand(
      ctx,
      systemTeacherId,
    );

    // The hand-authored "AI Literacy" strand — two grade-banded units (grade 2
    // and grades 3-5) that prebunk engagement-maximizing companion AIs. Uses
    // Rabbithole's own honest tutor as the contrast case. Idempotent by slug.
    // See convex/seedAiLiteracyUnits.ts + review/anti-parasocial-design.md.
    const aiLiteracyUnitCount = await insertAiLiteracyUnits(
      ctx,
      systemTeacherId,
    );

    // The "Geography Quests" unit — three map-driven social-studies quests
    // (Oʻahu wet/dry side, finding Washington DC, WWI causes) on the GeoMap
    // surface. Idempotent by slug. See convex/seed/geographyQuests.ts +
    // review/geography-quest-mapbox-plan.html.
    const geographyQuestUnitCount = await insertGeographyQuests(
      ctx,
      systemTeacherId,
    );

    // The games-platform demo unit — ONE lesson holding a game activity then an
    // ordinary online debrief, which is the entire mechanism behind "no
    // mandatory debrief phase inside the game". See convex/seed/gamesDemo.ts.
    const gamesDemoUnitCount = await insertGamesDemoUnit(ctx, systemTeacherId);
    const systemsAgentsResult = await insertSystemsAgentsUnit(ctx, systemTeacherId);
    const cooperationConflictResult = await insertCooperationConflictUnit(ctx, systemTeacherId);

    console.log(
      `Seeded: 12 personas, 18 perspectives, ${KEEP_SHELL_UNITS.size} shell units + ${prodUnitCount} complete prod units + ${probabilityUnitCount} probability units + ${aiLiteracyUnitCount} AI-literacy units + ${geographyQuestUnitCount} geography-quest units + ${gamesDemoUnitCount} games-demo units + ${systemsAgentsResult.activitiesCreated} Systems & Agents activities + ${cooperationConflictResult.activitiesCreated} Cooperation & conflict activities, building-block refs, 1 system teacher, 8 test users`
    );
  },
});
      ...
      (await ctx.db
        ...
        .filter((q) =>
          q.or(
            ...
            q.eq(q.field("role"), ROLES.PLATFORM_ADMIN)
          )
        )
        .first());
    if (!teacher) {
...
export const patchAvatars = internalMutation({
  handler: async (ctx) => {
    const avatarMap: Record<string, { image: string; readingLevel?: string }> = {
      "test.teacher@moli.school": { image: "/avatars/teacher.png" },
      ...
      if (user.email) {
        ...
        if (isFake) {
          ...
          if (!user.username && !patch.username) {
            // Underscore the local part: "First Last@local" would otherwise
            // re-derive exactly the spaced username this cleanup exists to
            // remove (see migrations.normalizeSpacedUsernames).
            const derived = underscoreUsername(user.email.replace(/@.*$/, ""));
  ...
  handler: async (ctx) => {
    ...
    const createUnit = async (def: UnitDef) => {
      const unitId = await ctx.db.insert("units", {
        ...
        bigIdea: def.bigIdea,
        essentialQuestions: toKeyedGranules(def.essentialQuestions, undefined, "eq"),
        enduringUnderstandings: toKeyedGranules(def.enduringUnderstandings, undefined, "eu"),
        isActive: true,
      });
...
/**
 * Seed realistic health records for one-sheet screenshot testing.
 * Creates a signed health record for `test-scholar-001` (full record, with
 * allergies, medications, HAP, and `publicMediaOptOut: true` to verify the
 * photo still renders on the emergency one-sheet regardless of opt-out).
 * `test-scholar-002` intentionally receives no record (placeholder scenario).
 */
export const seedOneSheetHealthRecords = internalMutation({
  handler: async (ctx) => {
    const scholar = await ctx.db
      .query("users")
      .filter((q) => q.eq(q.field("username"), "test-scholar-001"))
      .first();
    if (!scholar) {
      console.log("test-scholar-001 not found; run seedAll first.");
      return;
    }
    // Skip if already seeded
    const existing = await ctx.db
      .query("scholarHealthRecords")
      .filter((q) => q.eq(q.field("scholarId"), scholar._id))
      .first();
    if (existing) {
      console.log("Health record for test-scholar-001 already exists.");
      return;
    }
    // Use the admin (avery) as the guardian/signer for seed purposes
    const admin = await ctx.db
      .query("users")
      .filter((q) => q.eq(q.field("username"), "avery"))
      .first();
    const guardianId = admin?._id ?? scholar._id;
    ...
    await ctx.db.insert("scholarHealthRecords", {
      ...
      childPreferredName: "Alex",
      childDob: "2015-03-14",
      childGrade: "4th",
      homeAddress: "123 Kailua Rd, Kailua, HI 96734",
      streetAddress: "123 Kailua Rd",
      city: "Kailua",
      state: "HI",
      zipCode: "96734",
      ...
      // Medical providers
      physicianName: "Dr. Keiko Tanaka",
      physicianPhone: "(808) 555-0142",
      dentistName: "Dr. James Loo",
      dentistPhone: "(808) 555-0188",
      insurancePlan: "Aloha Care",
      insuranceId: "AC-2024-789456",

      // Guardian 1
      guardian1Name: "Maria Rivera",
      guardian1Relationship: "mother",
      guardian1RelationshipOther: "",
      guardian1Phone: "(808) 555-0101",
      guardian1WorkPhone: "(808) 555-0210",
      guardian1Email: "mrivera@example.com",
      guardian1Employer: "Kailua Surf Co.",
      ...
      guardian2: {
        ...
        relationship: "father",
        phone: "(808) 555-0102",
        workPhone: "(808) 555-0310",
        email: "crivera@example.com",
        employer: "Honolulu Architects LLC",
      },
      custodyNotes: "",
      ...
      emergencyContacts: [
        ...
        },
        {
          ...
          relationship: "Uncle",
          phone: "(808) 555-0303",
          altPhone: "(808) 555-0404",
          canPickUp: false,
        },
      ],

      // Allergies — peanut (severe) + bee sting (moderate); EpiPen on file
      noKnownAllergies: false,
      allergies: [
        {
          allergen: "Peanuts",
          type: "food",
          reaction: "Hives, throat swelling, anaphylaxis",
          severity: "severe",
          emergencyTreatment: "Administer EpiPen immediately. Call 911.",
          epipenOnFile: true,
        },
        {
          allergen: "Bee stings",
          type: "insect",
          reaction: "Localized swelling, hives",
          severity: "moderate",
          emergencyTreatment: "Benadryl 25mg. Monitor for 30 minutes.",
          epipenOnFile: false,
        },
      ],
      allergyNotes: "EpiPen is kept in the school health office and in Alex's backpack at all times.",

      // Chronic conditions
      noChronicConditions: false,
      chronicConditions: ["asthma"],
      chronicConditionDetails: "Exercise-induced. Uses rescue inhaler before PE and outdoor activities.",
      ...
      medications: [
        {
          ...
          dosage: "2 puffs",
          frequency: "As needed before exercise or if symptomatic",
          administrationInstructions: "Shake well. Two puffs 15 minutes before exercise.",
          storedAtSchool: true,
      ...
      // Healthcare action plan — allergy + asthma HAP
      hap: {
        allergy: true,
        asthma: true,
        seizure: false,
        ...
        otherDesc: "",
        none: false,
        notes: "Allergy HAP on file in health office. Asthma action plan reviewed Sept 2024.",
      },
      ...
      // Media / restrictions
      // publicMediaOptOut TRUE here — per spec the photo must still render on
      // the one-sheet (opt-out restricts public/social use only, not internal
      // staff operational tools like this emergency card).
      ...
      cookingWaiverParentFullName: "Maria Rivera",
      cookingWaiverStudentFullName: "Alex Rivera",
      cookingWaiverDetails: "Peanut allergy and asthma; action plans are on file.",
      cookingWaiverDate: "2026-08-14",

      // Signer
      signerName: "Maria Rivera",
      signerAgreement: true,
    });

    console.log("Seeded health record for test-scholar-001 (Alex Rivera, publicMediaOptOut=true).");
  },
});
