...
// The proposal generator identifies with the SCHOLAR's actual institution, not
// a hardcoded primary-school name — its output becomes directives/seeds that can reach
// the scholar's tutor. The school name is threaded in from
// `institutionPromptProfileForScholar` (resolved in aiGetProposalContext); with
// the configured primary default this renders byte-for-byte as before.
export function buildProposalSystemPrompt(
  profile: InstitutionPromptProfile = DEFAULT_INSTITUTION_PROMPT_PROFILE,
): string {
  return `You are a gifted-education curriculum designer at ${profile.schoolName}. A
...
export const runProposal = internalAction({
  ...
  handler: async (ctx, args): Promise<ProposalOutput> => {
    ...
    const institutionId = await ctx.runQuery(
      ...
      { userId: ctxData.document.scholarId, principal: "scholar" },
    );

    const userMessage = buildUserMessage({
      redactedSummary: ctxData.document.redactedSummary,
      keyFindings: ctxData.document.keyFindings,
    ...
    const response = await anthropic.messages.create({
      model: MODELS.SONNET,
      max_tokens: 4096,
      system: buildProposalSystemPrompt(ctxData.institutionProfile),
      tools: [PROPOSAL_TOOL],
