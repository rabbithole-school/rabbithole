...
export const list = authedQuery({
  ...
  handler: async (ctx) => {
    const canManageCurriculum =
      isCurriculumRole(ctx.user.role);
...
export const update = curriculumMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const perspective = await ctx.db.get(id);
    if (!perspective) throw new Error("Perspective not found");
    requireAuthorOrPlatformAdmin(ctx.user, perspective);
...
export const deactivate = curriculumMutation({
  args: { id: v.id("perspectives") },
  handler: async (ctx, args) => {
    const perspective = await ctx.db.get(args.id);
    if (!perspective) throw new Error("Perspective not found");
    requireAuthorOrPlatformAdmin(ctx.user, perspective);
