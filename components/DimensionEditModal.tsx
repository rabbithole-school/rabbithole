...
import { Plus, Trash, DotsSixVertical, Scroll } from "@phosphor-icons/react";
...
interface DimensionEditModalProps {
  ...
  /** Option lists for unit building blocks (personas DEPRECATED — anti-parasocial) */
...
export function DimensionEditModal({
  ...
  useEffect(() => {
    if (open) {
      nextIdRef.current = 0;
      // eslint-disable-next-line react-hooks/set-state-in-effect -- intentional reset of form state when modal opens or data changes
      setFormData({
        title: data?.title ?? "",
      ...
      if (isEditing && data) {
        ...
        if (dimensionType === "persona") {
          ...
          await updateUnit({
            ...
            // personaId intentionally omitted (DEPRECATED — anti-parasocial).
            // Existing unit→persona links are left untouched. See TODO.html.
              ...
              <VStack gap={4} align="stretch">
                ...
                >
                  <Textarea
                    ...
                    placeholder={dimensionType === "persona"
                      ...
                      : "Instructions for the AI tutor for this session."}
                    rows={10}
                    fontFamily="body"
                    fontSize="md"
                    bg="white"
                ...
                {dimensionType === "unit" && (
                  <>
                    ...
                    {(perspectives?.length || processes?.length) ? (
                      <FieldRow
                        label="Building blocks"
                        hint="Optional perspective and process applied when this unit is active."
                      >
