...
// reachable from every staff tab. The live `listScholars` subscription is gated on
// `isOpen` so it never runs in the layout's steady state (avoids competing with
// route-transition renders); the low-frequency curriculum lists are passed in.
...
function buildEntries(
  ...
  return [
    ...scholars.filter((s) => match(s.name ?? "")).map((s) => ({
      ...
      href: `/teacher/scholars/${s.username ?? s.id}`,
      label: s.name ?? "Scholar", sublabel: s.lastSessionTitle ?? undefined, scholar: s,
    })),
...
export function CommandPalette({
  ...
  const skills =
    (useQuery(
      ...
      searchSkillsQueryArgs(canSearchSkills, isOpen, query),
    ) ?? EMPTY_SKILLS);

  const results = useMemo(
    () => (
      isOpen
        ? buildEntries(scholars, units, perspectives, processes, skills, query)
        : EMPTY_RESULTS
    ),
    [isOpen, scholars, units, perspectives, processes, skills, query],
  );
          ...
          <Dialog.Content maxW="480px" borderRadius="xl" shadow="2xl" overflow="hidden" p={0} mt={0}>
            ...
            <Box px={4} py={3} borderBottom="1px solid" borderColor="gray.200">
              <Input
                ...
                placeholder="Jump to a scholar, curriculum, or math skill…"
                border="none"
                fontFamily="heading"
                fontSize="md"
            ...
            <Box maxH="360px" overflowY="auto">
              {results.length === 0 ? (
                ...
                results.map((item, i) => (
                  ...
                  >
                    {item.kind === "scholar" ? (
                      <Avatar size="sm" name={item.scholar.name} src={item.scholar.image || undefined} colorKey={item.scholar.id} />
                    ) : (
