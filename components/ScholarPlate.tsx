 ...
 *  focus is live (or in the teacher's remote view). The type + the lock
 *  decision (`isLockedByFocus`) live in lib/focusLock.ts so both the web plate
 *  surfaces and the vitest suite share one policy. */
export type { PlateFocusLock };

/** The quiet lock affordance that replaces the "Start"/"Continue" CTA on a
 *  focus-locked card — a small lock, no new colour (per visual-design.md). */
function FocusLockCta() {
  return (
    <Box
      as="span"
      display="inline-flex"
...
/** The "After you finish: <focus>" line shown in a locked card's meta slot. */
function FocusLockMeta({ label }: { label: string | null }) {
  return (
    <HStack gap={1} fontSize="xs" color="charcoal.400" fontFamily="heading">
      <Lock size={12} />
      <Text lineClamp={1}>
        {label ? `After you finish: ${label}` : "After you finish your class focus"}
      </Text>
    </HStack>
  );
}
...
export function ScholarPlate({
  ...
  // at least the FIRST Welcome beat — the same "quiet lock" treatment class
  // focus already gets (FocusLockCta / the "After you finish" hint), just a
  // second reason to show it. Lifts the moment `completedCount` reaches 1;
  // fully gone once onboarding itself completes (`result.onboarding` → null).
...
function PlateSection({
   ...
   *  yet — gates the SAME "Custom Quest" + suggested-quests/peer-trails
   *  extras the class-focus lock already gates, with its own quiet copy. */
  welcomeGate?: boolean;
  unitPinAction?: (
    unitId: Id<"units">,
    title: string,
  ...
  // A live class focus locks NEW independent work: a fresh quest / suggested
  // seed opens an anchorless (out-of-focus) session, so it would land on the
  // read-only wall. Gate the Custom Quest entry and hide the explore extras
  // instead of letting the kid finish creation into that wall. A not-yet-
  // completed first Welcome beat locks the SAME entries for the SAME reason
  // — a zero-history scholar authoring/exploring before finishing even one
  // beat of Welcome (H1) — with its own quiet copy below.
  const focusLocked = !!focusLock?.unitId;
  const locked = focusLocked || welcomeGate;
  const lockedHint = focusLocked
    ? (focusLock?.label ? `Finish ${focusLock.label} first` : "Finish your class focus first")
    : "Finish Welcome first";
  // The lock only ever disables the "Custom Quest" authoring entry (and hides the
  // explore extras) — it NEVER hides an already-started or offered quest, which
  // stay playable right below. So the note must speak only to what's actually
  // locked (creating a Custom Quest); "New quests open up after…" over a
  // playable quest read as a contradiction (pilot9 J8b).
  const lockedNote = focusLocked
    ? (focusLock?.label
        ? `Custom Quests unlock after you finish: ${focusLock.label}`
        : "Custom Quests unlock after you finish your class focus")
    : "Custom Quests unlock after you finish Welcome";
  ...
  return (
    <Stack gap={4} id={`plate-${section.origin}`} scrollMarginTop="64px">
      ...
      {/* Focus-lock (or, before the first Welcome beat, welcome-lock) hint for
          the disabled Custom Quest entry — the visible twin of the locked
          cards' "After you finish" line (a tooltip alone isn't discoverable).
          Quiet: a small lock, muted text, no new colour. */}
      {section.origin === "is" && showExplore && locked && (
        <HStack gap={1.5} fontSize="xs" color="charcoal.400" fontFamily="heading">
          <Lock size={12} />
          <Text lineClamp={1}>{lockedNote}</Text>
        </HStack>
      )}
      ...
      {/* Suggested destinations (seeds) + social-proof peer trails sit under
          ...
          remote view). Hidden while a class focus is live (each opens an
          out-of-focus session that would land on the read-only wall) or
          before the first Welcome beat is done (H1: a zero-history scholar
          shouldn't be nudged into exploring before finishing even one beat).
...
function ISUnitCard({ unit, onOpen, focusLock }: {
  unit: ISUnitCardData;
  onOpen: (unitId: Id<"units">) => void;
  focusLock: PlateFocusLock;
}) {
  const locked = isLockedByFocus(focusLock, unit.unitId);
  ...
  return (
    <ActivityCard
      ...
      onClick={locked ? undefined : () => onOpen(unit.unitId)}
      ...
      cta={
        locked ? (
          ...
          <ActivityCardCta>{inProgress ? "Continue" : "Start"}</ActivityCardCta>
        )
      }
    >
      ...
      {locked ? (
        <FocusLockMeta label={focusLock?.label ?? null} />
      ) : (
...
function PlateRowCard({
  ...
  // Homework-origin cards are exempt from the focus lock (policy a) — passed
  // through so isLockedByFocus can keep required homework startable during a
  // live class focus. IS/Quest + classFocus rows still lock.
  ...
  const href =
    row.sessionId && !locked && !isOfflineHomework
