...
public class MTFontManager {
    ...
    public var defaultFont: MTFont? {
        // Only the Fira Math face is bundled in mathFonts.bundle (Latin Modern
        // was dropped). SwiftMath's MTMathUILabel.initCommon() eagerly loads
        // this default font at construction — before ExpoMathView.applyFont()
        // can substitute Fira — and MTFont(fontWithName:) force-unwraps a
        // missing font file, so a Latin Modern default hard-crashes every math
        // label. Default to the bundled Fira face instead.
        MTFontManager.fontManager.firaRegularFont(withSize: kDefaultFontSize)
    }
