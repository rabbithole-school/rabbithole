 ...
 *     "soon".
 *   - Nothing is yanked at `endsAt` — the lock lifting is the ONLY mechanical
 *     effect; the scholar's session and work are untouched and still open.
...
/**
 * The read-only "your class is elsewhere" banner (item 1) — shown when the
 * scholar is viewing a DIFFERENT session than the live class focus. Composer
 * disable/enable is the caller's concern; this only builds the sentence.
 */
export function focusMismatchBannerText(
  ...
  if (phase === "windingDown") {
    return timeLabel
      ? `Right now ${who} — the room moves on soon (~${timeLabel}). You can read this session; sending here can wait till then.`
      : `Right now ${who} — the room moves on soon. You can read this session; sending here can wait till then.`;
  }
  return timeLabel
    ? `Right now ${who}, until the block wraps (~${timeLabel}). You can read this session; sending here can wait till then.`
    : `Right now ${who}. You can read this session; sending here can wait till then.`;
}
