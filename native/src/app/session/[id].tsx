...
export default function SessionScreen() {
  ...
  // winding-down cue (item 2) and the at-the-turn choice (item 3), never a
  // lock — nothing here disables the composer or the session.
          ...
          ListHeaderComponent={
            <View>
              <GeoMapCard
                ...
                commitBusy={tutorBusy || isFocusMismatch}
                onCommit={(text) => {
        ...
        {voice.isRecording || voice.isTranscribing ? (
          ...
          <View style={styles.composerRow}>
            ...
            <Pressable
              ...
              disabled={tutorBusy || isFocusMismatch}
              accessibilityRole="button"
            ...
            >
              <SymbolView
                ...
                size={32}
                tintColor={tutorBusy || isFocusMismatch ? colors.gray300 : colors.charcoalSubtle}
              />
            </Pressable>
            ...
            <View style={styles.inputWrap}>
              <AppTextInput
                ...
                editable={!tutorBusy && !isFocusMismatch}
              />
              ...
              {!canSend && !image.imageId ? (
                ...
                >
                  <SymbolView
                    ...
                    size={38}
                    tintColor={tutorBusy || isFocusMismatch ? colors.gray300 : colors.violet}
                  />
                </Pressable>
              ) : (
                <Pressable
    ...
    <>
      ...
      {isFocusMismatch ? (
        <View style={styles.focusBanner}>
          <SymbolView name="lock.fill" size={16} tintColor={colors.orange} />
      ...
      {showPanelLayout ? (
        ...
        <View style={styles.landscapeRow}>
          ...
          <DeliverablePanel
            sessionId={sessionId}
            activityId={activityId}
            checkDisabled={tutorBusy || isFocusMismatch}
            onAskCheck={(artifact, shouldCheck) =>
              onRubricCheck(artifact._id, artifact.title, shouldCheck)
            }
          />
...
function makeStyles(c: ReturnType<typeof useColors>) {
  ...
  focusBanner: {
    ...
    paddingVertical: 10,
    borderBottomWidth: 1,
    borderBottomColor: palette.orange[200],
  ...
  // "The turn, not the bell" — calm, non-alarming violet, distinct from the
  // orange lock banner: nothing here is locked, just a soft heads-up.
  roomTurnBanner: {
    flexDirection: "row",
    alignItems: "center",
    gap: 9,
